...
if (-not $SkipBuild) {
    ...
    if ($vsixSize -lt 700000) {
        Write-Warning "VSIX is suspiciously small ($vsixSize bytes). Expected ~900KB+. node_modules might be missing!"
    }
} else {
...
# Also clean up legacy publisher if present
$legacyId = "pesosz.antigravity-auto-accept"
Get-ChildItem $extensionsDir -Filter "$legacyId-*" -Directory | ForEach-Object {
    Write-Host "  Removing legacy publisher: $($_.Name)"
    Remove-Item $_.FullName -Recurse -Force
}
...
# Remove old entries for our extension and legacy publisher
$extensions = @($extensions | Where-Object { $_.identifier.id -ne $extId -and $_.identifier.id -ne $legacyId })
...
$newEntry = [PSCustomObject]@{
    ...
    location = [PSCustomObject]@{
        '$mid' = 1
        path = "/c:/Users/$($env:USERNAME)/.antigravity/extensions/$targetDirName"
        scheme = "file"
    }
